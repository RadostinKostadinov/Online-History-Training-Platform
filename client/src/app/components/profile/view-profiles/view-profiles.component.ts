...
export class ViewProfilesComponent implements OnInit, OnDestroy {
  ...
  maxPageNumber: number = Math.ceil(
    this.currentClass.length / this.usersPerPage
  );

  constructor(
    private vpService: ViewProfilesService,
  ...
  ngOnInit(): void {
    ...
    this.allStudentsSubs = this.vpService.students.subscribe(
      (students: any) => {
    ...
    this.classSelectSub = this.avClassesForm
      .get('avClass')
  ...
  updateTable() {
    if (
      this.pageNumber * this.usersPerPage + this.usersPerPage >
      this.currentClass.length
    ) {
      this.usersToShow = this.currentClass.slice(
