...
export class EditResourcesComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http
      .get(`${environment.backendUrl}eras/get/all`)
      .pipe(take(1))
      .subscribe((eras) => {
        this.eras = eras;
      });
  ...
  onChooseLesson(eraId: string, lessonId: string) {
    ...
    this.http
      .get(`${environment.backendUrl}lessons/get/${this.lesson._id}`)
      .pipe(take(1))
  ...
  onAddLesson(event: any, eraId: string) {
    ...
    const newLessonName = (<HTMLInputElement>(
      event.target.parentElement.querySelector('#add-new-lesson-input')
    ))!.value;
    this.http
      .post(`${environment.backendUrl}lessons/create`, {
        name: newLessonName,
      })
      .pipe(take(1))
      .subscribe({
        next: (res: any) => {
          ...
          this.http
            .get(`${environment.backendUrl}lessons/get/${res.lessonId}`)
            .pipe(take(1))
            .subscribe((lesson: any) => {
              this.lesson = lesson;
              this.http
                .patch(`${environment.backendUrl}eras/update/${this.era._id}`, {
  ...
  onDeleteLesson(era: any, lessonId: string) {
    this.http
      .delete(`${environment.backendUrl}lessons/delete/${lessonId}`)
      .pipe(take(1))
      .subscribe({
        next: (res) => {
  ...
  onDeleteItem(event: any, lessonItemId: string) {
    event.preventDefault();
    this.http
      .delete(`${environment.backendUrl}lessonItems/delete/${lessonItemId}`)
      .pipe(take(1))
      .subscribe({
        next: (res) => {
  ...
  onChooseLessonItem(event: any, lessonItemId: string) {
    ...
    this.http
      .get(`${environment.backendUrl}lessonItems/get/${lessonItemId}`)
      .pipe(take(1))
      .subscribe((lessonItem: any) => {
  ...
  addNewLessonItem(event: any) {
    ...
    //get new item name
    const newLessonItemName = (<any>(
      document.getElementById('new-lesson-item-input')
    ))?.value;

    //create new item with this name
    this.http
      .post(`${environment.backendUrl}lessonItems/create`, {
  ...
  onVideoUpload(event: any) {
    ...
    if (newVideoUrl.includes('https://www.youtube.com/')) {
      ...
      this.itemVideos.set(
        newVideoUrl,
        this.sanitizer.bypassSecurityTrustResourceUrl(newVideoUrl)
      );
    }
  }
  ...
  saveItem(event: any) {
    event.preventDefault();
    this.lessonItem.date = (<any>(
      document.querySelector('.third-screen-lesson-date')
    )).value;
    this.lessonItem.text = (<any>(
      document.querySelector('#lessonItem-text')
    )).value;
    this.lessonItem.interesting = (<any>(
      document.querySelector('#lessonItem-interesting')
    )).value;
    ...
    this.http
      .patch(
        `${environment.backendUrl}lessonItems/update/${this.lessonItem._id}`,
        this.lessonItem
      )
      .pipe(take(1))
      .subscribe({
